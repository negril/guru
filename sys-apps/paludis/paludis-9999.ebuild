...
if [[ ${PV} == *9999* ]] ; then
	...
	if [[ ${PV} == *_beta* ]] ; then
		SRC_URI="https://github.com/MageSlayer/paludis-gentoo-patches/archive/refs/tags/v${PV/_/-}.tar.gz -> ${P}.tar.gz"
		S="${WORKDIR}"/${P/_/-}
	else
		SRC_URI="https://github.com/MageSlayer/paludis-gentoo-patches/releases/download/v${PV}/${P}.tar.gz"
		S="${WORKDIR}/${PN}"
		KEYWORDS="~amd64 ~x86"
	fi
fi
